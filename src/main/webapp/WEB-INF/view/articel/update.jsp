	 ...
	 <form action="" id="form">
		...
		<div class="form-group row ">
			<textarea name="content1" cols="100" rows="8"
				style="width: 860px; height: 250px; visibility: hidden;">${article.content}</textarea>
			<br />
		</div>
		<div class="form-group row ">
			<label for="title">文章标题图片</label> <input type="file"
				class="form-control" id="file" name="file" value="${article.picture}">
		</div>
		<div class="form-group row ">
	   ...
	   $.post("/user/getCategoryByChannel",{chnId:channelId},function(data){
		   ...
		   if(data.num==1){
			   ...
			   for ( var index in data.date) {
				   if(data.date[index].id == ${article.categoryId} ){
				   	  $("#category").append("<option value='"+ data.date[index].id +"' selected>"+data.date[index].name+"</option>");
				  }else{
